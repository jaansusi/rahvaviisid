...
@model({
  settings: {idInjection: false, postgresql: {schema: 'folk_tune', table: 'tune_states'}}
})
export class TuneStates extends Entity {
  @property({
    type: 'number',
    required: false,
    scale: 0,
  ...
  @hasMany(() => Tunes, {keyFrom: 'id', keyTo: 'tuneStateId'})
  tunes: Tunes[];
  ...
  constructor(data?: Partial<TuneStates>) {
    super(data);
  }
}

export interface TuneStatesRelations {
  // describe navigational properties here
}

export type TuneStatesWithRelations = TuneStates & TuneStatesRelations;
