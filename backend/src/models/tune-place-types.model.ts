...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'folk_tune', table: 'tune_place_types'}
  }
})
export class TunePlaceTypes extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<TunePlaceTypes>) {
    super(data);
  }
}

export interface TunePlaceTypesRelations {
  // describe navigational properties here
}

export type TunePlaceTypesWithRelations = TunePlaceTypes & TunePlaceTypesRelations;
