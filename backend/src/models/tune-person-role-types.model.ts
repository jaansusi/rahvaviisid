...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'folk_tune', table: 'tune_person_role_types'}
  }
})
export class TunePersonRoleTypes extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<TunePersonRoleTypes>) {
    super(data);
  }
}

export interface TunePersonRoleTypesRelations {
  // describe navigational properties here
}

export type TunePersonRoleTypesWithRelations = TunePersonRoleTypes & TunePersonRoleTypesRelations;
