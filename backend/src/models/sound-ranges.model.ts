...
@model({
  settings: {idInjection: false, postgresql: {schema: 'folk_tune', table: 'sound_ranges'}}
})
export class SoundRanges extends Entity {
  @property({
    type: 'number',
    required: false,
    scale: 0,
  ...
  @hasMany(() => Tunes, {through: {
    model: () => MusicalCharacteristics,
    keyFrom: 'soundRangeId',
    keyTo: 'tunesId'
  }})
  tunes?: Tunes[];
  ...
  constructor(data?: Partial<SoundRanges>) {
    super(data);
  }
}

export interface SoundRangesRelations {
  // describe navigational properties here
}

export type SoundRangesWithRelations = SoundRanges & SoundRangesRelations;
