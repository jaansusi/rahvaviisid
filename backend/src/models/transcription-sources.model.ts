...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'folk_tune', table: 'transcription_sources'}
  }
})
export class TranscriptionSources extends Entity {
  @property({
    type: 'number',
    required: false,
    scale: 0,
  ...
  constructor(data?: Partial<TranscriptionSources>) {
    super(data);
  }
}

export interface TranscriptionSourcesRelations {
  // describe navigational properties here
}

export type TranscriptionSourcesWithRelations = TranscriptionSources & TranscriptionSourcesRelations;
